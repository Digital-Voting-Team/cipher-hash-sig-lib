...
func Modsqrt(a, p int) int {
	...
	if legendreSymbol(a, p) != 1 {
		...
		return int(math.Pow(float64(a), float64((p+1)/4))) % p
	}
	...
	x := int(math.Pow(float64(a), float64((s+1)/2))) % p
	b := int(math.Pow(float64(a), float64(s))) % p
	g := int(math.Pow(float64(n), float64(s))) % p
	r := e

	for {
		t, m := b, 0
		for _, m = range makeRange(0, r) {
			if t == 1 {
				break
			}
			t = int(math.Pow(float64(t), 2)) % p
		}
		if m == 0 {
			return x
		}

		gs := int(math.Pow(float64(g), math.Pow(2, float64(r-m-1)))) % p
		g = (gs * gs) % p
		x = (x * gs) % p
		b = (b * g) % p
		r = m
...
func legendreSymbol(a, p int) int {
	ls := int(math.Pow(float64(a), float64((p-1)/2))) % p
	if ls == p-1 {
		return -1
	}
	return ls
