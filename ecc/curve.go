...
func (mc *MontgomeryCurve) addPoint(P, Q Point) Point {
	...
	delta_x := *P.X - *Q.X
	delta_y := *P.Y - *Q.Y
	modInv, err := Modinv(delta_x, *mc.P)
	if err != nil {
		log.Fatal(err)
	}
	s := delta_y * modInv
	res_x := (*mc.B*s*s - *mc.A - *P.X - *Q.X) % *mc.P
	res_y := (*P.Y + s*(res_x-*P.X)) % *mc.P
	return (&Point{&res_x, &res_y, mc}).Neg()
}
...
func (mc *MontgomeryCurve) doublePoint(P Point) Point {
	...
	s := up * modInv
	res_x := (*mc.B*s*s - *mc.A - 2**P.X) % *mc.P
	res_y := (*P.Y + s*(res_x-*P.X)) % *mc.P
	return (&Point{&res_x, &res_y, mc}).Neg()
}
...
func (mc *MontgomeryCurve) ComputeY(x int64) int64 {
	...
	inv_b, err := Modinv(*mc.B, *mc.P)
	if err != nil {
		log.Fatal(err)
	}
	right = (right * inv_b) % *mc.P
